...
==================================
CSV (local) → MinIO raw/ (CSV) → MinIO processed/ (Parquet) → ClickHouse greenery.addresses
"""
...
def greenery_ingest_addresses():
    """Load addresses.csv → MinIO raw → Parquet → MinIO processed → ClickHouse."""
    ...
    def csv_to_parquet(raw_key: str, ds=None) -> str:
        ...
        processed_key = f"processed/addresses/{ds}/addresses.parquet"
        s3_hook = S3Hook(aws_conn_id=MINIO_CONN_ID)

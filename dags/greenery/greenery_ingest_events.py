"""
Greenery Events Ingest Pipeline
===============================
CSV (local) → MinIO raw/ (CSV) → MinIO processed/ (Parquet) → ClickHouse greenery.events
"""
...
from greenery.schemas.events import pyarrow_schema, clickhouse_ddl

events_asset = Asset("clickhouse://greenery/events")

MINIO_CONN_ID = "minio_default"
MINIO_BUCKET = "greenery"

CSV_PATH = "/opt/airflow/docs/greenery/events.csv"
...
@dag(
    schedule="@daily",
    dag_id="greenery_ingest_events",
    start_date=datetime(2026, 1, 1),
    tags=["greenery", "ingest", "events"],
    catchup=False,
    doc_md=__doc__,
)
def greenery_ingest_events():
    """Load events.csv → MinIO raw → Parquet → MinIO processed → ClickHouse."""

    @task
    def upload_csv_to_minio_raw(ds=None) -> str:
        """Read events.csv from local filesystem and upload to MinIO raw layer."""
        raw_key = f"raw/events/{ds}/events.csv"
    ...
    def csv_to_parquet(raw_key: str, ds=None) -> str:
        ...
        processed_key = f"processed/events/{ds}/events.parquet"
        s3_hook = S3Hook(aws_conn_id=MINIO_CONN_ID)
    ...
    def create_clickhouse_table():
        """Create greenery database and events table in ClickHouse if not exists."""

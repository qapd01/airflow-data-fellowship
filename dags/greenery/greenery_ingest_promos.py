"""
Greenery Promos Ingest Pipeline
===============================
CSV (local) → MinIO raw/ (CSV) → MinIO processed/ (Parquet) → ClickHouse greenery.promos
"""
...
from greenery.schemas.promos import pyarrow_schema, clickhouse_ddl

promos_asset = Asset("clickhouse://greenery/promos")

MINIO_CONN_ID = "minio_default"
MINIO_BUCKET = "greenery"

CSV_PATH = "/opt/airflow/docs/greenery/promos.csv"

CLICKHOUSE_CONN_ID = "clickhouse_conn"
CLICKHOUSE_DB = "greenery"
CLICKHOUSE_TABLE = "promos"


@dag(
    schedule="@daily",
    dag_id="greenery_ingest_promos",
    start_date=datetime(2026, 1, 1),
    tags=["greenery", "ingest", "promos"],
    catchup=False,
    doc_md=__doc__,
)
def greenery_ingest_promos():
    """Load promos.csv → MinIO raw → Parquet → MinIO processed → ClickHouse."""

    @task
    def upload_csv_to_minio_raw(ds=None) -> str:
        """Read promos.csv from local filesystem and upload to MinIO raw layer."""
        raw_key = f"raw/promos/{ds}/promos.csv"
    ...
    def csv_to_parquet(raw_key: str, ds=None) -> str:
        ...
        processed_key = f"processed/promos/{ds}/promos.parquet"
        s3_hook = S3Hook(aws_conn_id=MINIO_CONN_ID)
    ...
    def create_clickhouse_table():
        """Create greenery database and promos table in ClickHouse if not exists."""

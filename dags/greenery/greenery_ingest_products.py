"""
Greenery Products Ingest Pipeline
=================================
CSV (local) → MinIO raw/ (CSV) → MinIO processed/ (Parquet) → ClickHouse greenery.products
"""
...
from greenery.schemas.products import pyarrow_schema, clickhouse_ddl

products_asset = Asset("clickhouse://greenery/products")

MINIO_CONN_ID = "minio_default"
MINIO_BUCKET = "greenery"

CSV_PATH = "/opt/airflow/docs/greenery/products.csv"
...
@dag(
    schedule="@daily",
    dag_id="greenery_ingest_products",
    start_date=datetime(2026, 1, 1),
    tags=["greenery", "ingest", "products"],
    catchup=False,
    doc_md=__doc__,
)
def greenery_ingest_products():
    """Load products.csv → MinIO raw → Parquet → MinIO processed → ClickHouse."""

    @task
    def upload_csv_to_minio_raw(ds=None) -> str:
        """Read products.csv from local filesystem and upload to MinIO raw layer."""
        raw_key = f"raw/products/{ds}/products.csv"
    ...
    def csv_to_parquet(raw_key: str, ds=None) -> str:
        ...
        processed_key = f"processed/products/{ds}/products.parquet"
        s3_hook = S3Hook(aws_conn_id=MINIO_CONN_ID)
    ...
    def create_clickhouse_table():
        """Create greenery database and products table in ClickHouse if not exists."""
